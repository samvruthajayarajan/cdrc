      ...
      <div style={{ background: '#fff', borderRadius: '1.25rem', width: '100%', maxWidth: 500, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 24px 60px rgba(0,0,0,0.2)', fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${RB}, ${RBD})`, padding: '1.5rem 1.75rem', borderRadius: '1.25rem 1.25rem 0 0', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.875rem' }}>
            ...
            <div>
              <h2 style={{ color: '#fff', fontWeight: 900, fontSize: '1.1rem', margin: 0, lineHeight: 1.2 }}>Enrollment Inquiry</h2>
        ...
        <div style={{ padding: '1.75rem' }}>
          {status === 'success' ? (
            ...
            <div style={{ textAlign: 'center', padding: '1.5rem 1rem 2rem' }}>
              ...
              <h3 style={{ color: '#0f172a', fontSize: '1.3rem', fontWeight: 900, marginBottom: '0.5rem' }}>Inquiry Submitted!</h3>
              ...
              <div style={{ background: RBL, borderRadius: '0.875rem', padding: '1rem', marginBottom: '1.5rem', textAlign: 'left' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: RB, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.5rem' }}>Enrollment Details</div>
              ...
              <button onClick={onClose} style={{ padding: '0.75rem 2rem', background: RB, color: '#fff', border: 'none', borderRadius: '0.625rem', fontWeight: 700, fontSize: '0.88rem', cursor: 'pointer' }}>
                Close
              </button>
            </div>
          ) : (
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: '0.35rem' }}>Full Name <span style={{ color: '#ef4444' }}>*</span></label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: '0.35rem' }}>Email <span style={{ color: '#ef4444' }}>*</span></label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: '0.35rem' }}>Phone <span style={{ color: '#ef4444' }}>*</span></label>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', paddingTop: '0.25rem' }}>
                <button type="button" onClick={onClose}
                  style={{ flex: 1, padding: '0.75rem', borderRadius: '0.625rem', border: '1.5px solid #e2e8f0', background: '#fff', color: '#64748b', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#f8fafc'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#fff'; }}
                >Cancel</button>
                <button type="submit" disabled={status === 'loading'}
                  style={{ flex: 2, padding: '0.75rem', borderRadius: '0.625rem', background: status === 'loading' ? '#93c5fd' : RB, color: '#fff', border: 'none', fontWeight: 700, fontSize: '0.85rem', cursor: status === 'loading' ? 'not-allowed' : 'pointer', transition: 'background 0.2s', boxShadow: `0 4px 14px rgba(65,105,225,0.3)` }}
