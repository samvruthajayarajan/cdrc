...
export async function GET() {
  try {
    ...
    const questions = await db.collection('courseFinderQuestions').find({}).sort({ order: 1 }).toArray();
...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = await getDb();
    const result = await db.collection('courseFinderQuestions').insertOne({ ...body, createdAt: new Date() });
