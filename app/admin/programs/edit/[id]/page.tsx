...
import { ArrowLeft, Building, DollarSign, BookOpen, Globe, Award, Search, GraduationCap, Edit } from '@/components/Icon';
...
export default function EditProgramPage() {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`/api/programs/${id}`).then(r => r.json()).then(d => {
      if (d.success && d.data) {
        const p = d.data;
        setForm({
          name: p.name || '',
          ...
          syllabus: Array.isArray(p.syllabus) ? p.syllabus.join(', ') : p.syllabus || '',
          highlights: Array.isArray(p.highlights) ? p.highlights.join(', ') : p.highlights || '',
          careerOptions: Array.isArray(p.careerOptions) ? p.careerOptions.join(', ') : p.careerOptions || '',
      ...
      setFetching(false);
    }).catch(() => { setError('Failed to load program'); setFetching(false); });
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/programs/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
      if (data.success) { setSuccess('Program updated successfully! Redirecting...'); setTimeout(() => router.push('/admin/programs'), 1500); }
      else setError(data.error || 'Failed to update program');
    } catch { setError('Error updating program'); }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 40, height: 40, border: `4px solid #e2e8f0`, borderTop: `4px solid ${ACCENT}`, borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
          <p style={{ color: '#64748b' }}>Loading program...</p>
        </div>
      ...
      <main style={S.main}>
        <div style={S.container}>
          <div style={S.pageHeader}>
            ...
            <h1 style={S.pageTitle}>Edit Program</h1>
            <p style={S.pageSubtitle}>Update the details for <strong>{form.name || 'this program'}</strong></p>
          </div>
          ...
          {success && (
            <div style={S.successAlert}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ display: 'inline', marginRight: 8 }}><polyline points="20 6 9 17 4 12"/></svg>
              {success}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* Basic Info */}
            <div style={S.section}>
              <SectionHeader icon={<GraduationCap size={18} color="#fff" />} title="Basic Information" desc="Core details of the program" />
              <div style={S.grid2}>
                ...
                <div>
                  ...
                  <select {...selectProps('universityId')} onChange={e => { set('universityId', e.target.value); set('university', universities.find(u => u._id === e.target.value)?.name || form.university); }}>
            ...
            {/* Fee */}
            <div style={S.section}>
              <SectionHeader icon={<DollarSign size={18} color="#fff" />} title="Fee Information" desc="Program fee structure" />
              <div style={S.grid2}>
                <div>
            ...
            {/* Description */}
            <div style={S.section}>
              <SectionHeader icon={<BookOpen size={18} color="#fff" />} title="Description & Eligibility" desc="Detailed information about the program" />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={S.label}>Description</label>
                  <textarea {...textareaProps('description', 5)} placeholder="Provide a detailed description..." />
                  <p style={S.hint}>{form.description.length} characters</p>
                </div>
                <div>
                  <label style={S.label}>Eligibility Criteria</label>
                  <textarea {...textareaProps('eligibility', 3)} placeholder="e.g., Bachelor's degree with minimum 50% marks" />
                </div>
              </div>
            </div>
            ...
            {/* Program Details */}
            <div style={S.section}>
              <SectionHeader icon={<Award size={18} color="#fff" />} title="Program Details" desc="Syllabus, highlights, career options, and specializations" />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
            ...
            {/* Media */}
            <div style={S.section}>
              <SectionHeader icon={<Globe size={18} color="#fff" />} title="Media" desc="Program image, brochure, and video links" />
              <div style={S.grid2}>
                <div>
            ...
            {/* Settings */}
            <div style={S.section}>
              <SectionHeader icon={<Building size={18} color="#fff" />} title="Settings" desc="Visibility and featured status" />
              <div style={S.grid2}>
            ...
            <div style={S.actions}>
              ...
              <button type="submit" disabled={loading} style={{ ...S.submitBtn, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}>
                {loading ? 'Updating...' : <><Edit size={15} color="#fff" /> Update Program</>}
              </button>
            </div>
