  ...
  useEffect(() => {
    fetch('/api/skills').then(r => r.json()).then(d => {
      if (d.success) {
        ...
        if (skill) {
          setForm({ name: skill.name || '', description: skill.description || '', category: skill.category || 'Technology', level: skill.level || 'Beginner', duration: skill.duration || '', price: skill.price || '', image: skill.image || '' });
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      <div style={{ background: 'linear-gradient(135deg, #4361EE, #2d2d6b)', padding: '1.5rem 2rem', color: '#fff' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: '1.75rem', fontWeight: 900 }}>Edit Skill Program</h1>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <button type="submit" disabled={loading} style={{ padding: '0.75rem 1.75rem', background: loading ? '#94a3b8' : '#4361EE', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', fontSize: '0.9rem', fontFamily: 'inherit' }}>
